...
var jdd = {
    ...
    formatPRETags: function() {
        _.each($('pre'), function(pre) {
            ...
            var codeLines = $('<div class="display-div-diff"></div>');
    ...
    setupNewDiff: function() {
        $("#jsonInputView").removeClass("hide");
        $("#jsonDiffView").addClass("hide");
    },
    ...
    generateReport: function() {
        ...
        var newDiff = $('<button class="btn">Back to input</button>');
         ...
         */
        if (missingCount > 0) {
            var missing = $('<label class="display-label-diff"><input id="showMissing" type="checkbox" name="checkbox" value="value" checked="true"></label>');
            if (missingCount === 1) {
         ...
         */
        if (typeCount > 0) {
            var types = $('<label class="display-label-diff"><input id="showTypes" type="checkbox" name="checkbox" value="value" checked="true"></label>');
            if (typeCount === 1) {
         ...
         */
        if (eqCount > 0) {
            var eq = $('<label class="display-label-diff"><input id="showEq" type="checkbox" name="checkbox" value="value" checked="true"></label>');
            if (eqCount === 1) {
    ...
    compare: function() {
        ...
        var textarealeft = editorAce.getValue();
        var textarearight = editorResult.getValue();
        
         var leftValid = jdd.validateInput(textarealeft, jdd.LEFT);
         var rightValid = jdd.validateInput(textarearight, jdd.RIGHT);

        if (!leftValid || !rightValid) {
            $('body').removeClass('progress');
            $('#compare').prop('disabled', false);
            openErrorDialog("invalid json input");
            return;
        }
        ...
        var left = JSON.parse(textarealeft);
        var right = JSON.parse(textarearight);
        ...
        $("#jsonInputView").addClass("hide");
        $("#jsonDiffView").removeClass("hide");
    },
